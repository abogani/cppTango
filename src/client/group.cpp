...
DeviceNames GroupElementFactory::resolve_remote_device_names(int db_port,
                                                             std::string &db_host,
                                                             std::string &name_or_pattern_without_host)
{
...
DeviceNames
    GroupElementFactory::add_host_and_port_to_device_names(const DeviceNames &names, const std::string &host, int port)
{
...
std::string
    GroupElementFactory::build_full_device_name(const std::string &host, int port, const std::string &device_name)
{
...
void Group::dump(int indent_level)
{
    ...
    TANGO_LOG << "|- GROUP: " << get_fully_qualified_name() << " [" << get_size_i(false) << ":" << get_size_i(true)
              << "]" << std::endl;
    for(; it != end; ++it)
    {
...
//-----------------------------------------------------------------------------
long GroupDeviceElement::command_inout_asynch_i(
    const std::string &c, const DeviceData &d, bool fgt, TANGO_UNUSED(bool fwd), long id)
{
    if(!is_enabled())
    {
        if(!fgt)
    ...
    else
    {
        ...
        {
            long ari =
                dev_proxy()->command_inout_asynch(const_cast<std::string &>(c), const_cast<DeviceData &>(d), fgt);
            if(!fgt)
            {
                arp.insert(AsynchRequestRepValue(id, AsynchRequest(ari, c)));
            }
