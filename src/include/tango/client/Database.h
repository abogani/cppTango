...
class Database : public Tango::Connection
{
  ...
  public:
     ...
     * @exception ConnectionFailed,CommunicationFailed,DevFailed from device
     */
    std::vector<DbHistory> get_device_pipe_property_history(const std::string &dev_name,
     ...
     * @exception ConnectionFailed,CommunicationFailed,DevFailed from device
     */
    std::vector<DbHistory> get_class_pipe_property_history(const std::string &class_name,
                                                           const std::string &pipe_name,
                                                           const std::string &prop_name);
    //@}
     ...
     *
     * Query the database for a list of properties defined for the specified object and which match the specified
     * wildcard.
     * @code
     ...
     *
     * Get device alias list. The parameter is a std::string to filter the alias list returned. Wildcard (*) is
     * supported. For instance, if the std::string passed as the method parameter is initialised with only the *
     * character, all the defined device alias will be returned. The DbDatum returned by this method is initialised with
     * an array of std::strings and must be extracted into a std::vector<std::string>. If there is no alias with the
     ...
     * initialised with an array of std::strings and must be extracted into a std::vector<std::string>. If there is no
     * alias with the given filter, the returned array will have a 0 size.
     *
     * @param [in] filter The filter
    ...
    {
        return multi_db_host;
    }

    std::vector<std::string> &get_multi_port()
    {
        return multi_db_port;
    }

    const std::string &get_file_name();

    const std::string &get_orig_tango_host()
    {
        return ext->orig_tango_host;
    }

    void set_orig_tango_host(const std::string &_s)
    {
        ext->orig_tango_host = _s;
    }
