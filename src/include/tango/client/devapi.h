...
class DeviceDataHistory : public DeviceData
{
  public:
    ...
    void failed(bool val)
    {
        fail = val;
    }

    void set_date(TimeVal &tv)
    {
        time = tv;
    }
    ...
    const DevErrorList &errors()
    {
        return err.in();
    }

    void errors(DevErrorList_var &del)
    {
        err = del;
    }
 ...
 * all details regarding device polling.
 *
 *
 * @headerfile tango.h
 * @ingroup Client
...
class DummyDeviceProxy : public Tango::Connection
{
  public:
    ...
    int get_env_var(const char *cc, std::string &str_ref)
    {
        return Tango::Connection::get_env_var(cc, str_ref);
    }
};
