    ...
    else if(dynamic_cast<const CORBA::UserException *>(&e) != nullptr)
    {
        ...
        if((mdf = dynamic_cast<const Tango::NamedDevFailedList *>(&e)) != nullptr)
        {
            ...
            for(unsigned long i = 0; i < mdf->err_list.size(); i++)
            {
                ...
                {
                    os << "       Severity = " << mdf->err_list[i].err_stack[j].severity << std::endl;
            ...
            {
                os << "       Severity = " << mdf->errors[j].severity << std::endl;
        ...
        else if((te = dynamic_cast<const Tango::DevFailed *>(&e)) != nullptr)
        {
            for(unsigned long i = 0; i < te->errors.length(); i++)
            {
                ...
                os << "Severity = " << te->errors[i].severity << std::endl;
...
void Except::print_error_stack(const Tango::DevErrorList &e, std::ostream &os)
{
    for(unsigned long i = 0; i < e.length(); i++)
    {
        ...
        os << "Severity = " << e[i].severity << std::endl;
...
void Except::throw_named_exception(
    Tango::DeviceImpl *d, long ind, const char *reason, const char *desc, const char *origin, Tango::ErrSeverity sever)
{
    throw_named_exception(d->get_device_attr()->get_attr_by_ind(ind).get_name().c_str(), reason, desc, origin, sever);
}

void Except::throw_named_exception(Tango::DeviceImpl *d,
                                   std::vector<long> &ind_atts,
                                   const char *reason,
                                   const char *desc,
                                   const char *origin,
                                   Tango::ErrSeverity sever)
{
