...
void *PollThread::run_undetached(TANGO_UNUSED(void *ptr))
{
    ...
    while(true)
    {
        ...
        {
            std::cerr << "OUPS !! An unforeseen standard exception has been received by a polling thread !!!!!!"
                      << std::endl;
...
void PollThread::execute_cmd()
{
    ...
    case Tango::POLL_ADD_OBJ:
    {
        ...
        if(found == false)
        {
            ...
            if(wo.update != PollClock::duration::zero())
            {
                ...
                {
                    TANGO_LOG_DEBUG << "Received a delta from now of " << std::fixed << duration_ms(local_cmd.new_upd)
                                    << " ms" << std::endl;
    ...
    case Tango::POLL_UPD_PERIOD:
    {
        ...
        //
        // If not found in work list, it should be in the externally triggered object. Therefore, remove it from
        // externally triggered list and insert it in work list. If not found in work list and in trig list, we are in
    ...
    for(i = 0; i < nb_elt; i++)
    {
        if(ite->type != EVENT_HEARTBEAT)
        {
            if(ite->type != STORE_SUBDEV)
            {
                ...
                TANGO_LOG_DEBUG << "Dev name = " << ite->dev->get_name() << ", obj name = " << obj_list
                                << ", next wake_up at " << std::fixed
                                << duration_s(ite->wake_up_date.time_since_epoch()) << " s " << std::fixed << "(in "
                                << duration_ms(ite->wake_up_date - PollClock::now()) << " ms)" << std::endl;
            }
            else
            {
                TANGO_LOG_DEBUG << ite->name[0] << ", next wake_up at " << std::fixed
                                << duration_s(ite->wake_up_date.time_since_epoch()) << " s " << std::fixed << "(in "
                                << duration_ms(ite->wake_up_date - PollClock::now()) << " ms)" << std::endl;
            }
        }
        ...
        {
            TANGO_LOG_DEBUG << "Event heartbeat, next wake_up at " << std::fixed
                            << duration_s(ite->wake_up_date.time_since_epoch()) << " s " << std::fixed << "(in "
                            << duration_ms(ite->wake_up_date - PollClock::now()) << " ms)" << std::endl;
        }
...
void PollThread::tune_list(bool from_needed)
{
    ...
    //
    // If we try to tune the list with respect to works needed time, compute works needed time sum and find minimun
    // update period
    //
    ...
    if(works.empty() == false)
    {
        ...
        if(polling_bef_9 == false)
        {
            ...
            if(nb_late != 0)
            {
                ...
                {
                    TANGO_LOG_DEBUG << "Setting discard to true because nb_late == works.size() --> " << nb_late
                                    << std::endl;
                    discard = true;
                }
                else
...
void PollThread::poll_attr(WorkItem &to_do)
{
    ...
    // We also have to retrieve which kind of clients made the subscription (zmq or notifd) and send the event
    // accordingly
    //
    ...
    for(size_t ctr = 0; ctr < nb_obj; ctr++)
    {
        ...
        if((event_supplier_nd != nullptr) || (event_supplier_zmq != nullptr))
        {
            if(attr_failed == true)
            {
                ...
                //
                // Eventually push the event (if detected). When we have both notifd and zmq event supplier, do not
                // detect the event two times. The detect_and_push_events() method returns true if the event is
                // detected.
                //
