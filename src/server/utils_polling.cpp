...
void Util::polling_configure()
{
    ...
    TANGO_LOG_DEBUG << "POLLING: " << nb_thread << " thread(s) needed for polling from a pool of "
            ...
            catch(Tango::DevFailed &e)
            {
                ...
                if(throw_ex == true)
                {
                    ...
                    o << "Error when configuring polling for device "
                      << poll_ths[loop]->v_poll_cmd[cmd_loop]->svalue[0].in();
...
int Util::create_poll_thread(const char *dev_name, bool startup, bool polling_9, int smallest_upd)
{
    ...
    if(poll_ths.size() != poll_pool_size)
    {
        ...
        if(asso_devs.empty() != true)
        {
            ...
            //
            // If we find a thread for one of the associated device, no need to create a new one. Simply add the device
            // entry in the map <dev_name,thread id>
            //
    ...
    else
    {
        ...
        if(asso_devs.empty() != true)
        {
            ...
            //
            // If we find a thread for one of the associated device, simply add the device entry in the map
            // <dev_name,thread id>
            //
...
void Util::check_pool_conf(DServer *admin_dev, unsigned long pool_size)
{
    ...
    for(iter = mod_conf.begin(); iter != mod_conf.end(); ++iter)
    {
        ...
        for(iter_entry = splitted.begin(); iter_entry != splitted.end(); ++iter_entry)
        {
            ...
            //
            // The device is not defined in the DS, remove it from the conf. (Do not copy it in the new conf built by
            // this method)
            //
    ...
    if(mod_conf.size() > pool_size)
    {
        ...
        //
        // If we have more threads in the conf than in the pool, distribute the extra thread devices to the still
        // existing threads
        //
...
int Util::check_dev_poll(std::vector<std::string> &poll_cmd_list,
                         std::vector<std::string> &poll_attr_list,
                         DeviceImpl *dev)
{
    int ret = 0;
        ...
        while(retry == true)
        {
            try
            {
                ...
                else
                {
                    ...
                    // If we have only one thread, it is easy to catch this threshold. In case this threshold is
                    // reached, split entry in several sub-entries using the \ characters at the end of each sub-entries
                    //
