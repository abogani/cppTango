...
//
    ...
    auto split = [](const std::string& path_env,
        const std::string delimiter = ":") -> std::vector< std::string >
    {
        std::regex regex{delimiter};
        return {std::sregex_token_iterator(std::begin(path_env),
            std::end(path_env), regex, -1), std::sregex_token_iterator()};
    };
    ...
    auto file_exists_in_path = [&darwin_class_factory_symbol,
        &shortened_exe_name, &proc]
        (const std::string& path) -> bool
    {
        ...
        if((stat(full_path.c_str(), &exec_file_stat) == 0)
            &&  ((exec_file_stat.st_mode & S_IFDIR) == 0))
        {
            ...
            if((mod = dlopen(full_path.c_str(), RTLD_LAZY)) != nullptr)
            {
                ...
                if ((proc = dlsym(mod, darwin_class_factory_symbol.c_str()))
                    != nullptr)
                {
                    return true;
                }

                std::cerr << "Warning: When inspecting the file \""
                    << full_path
    ...
    auto paths_iterator{std::find_if(std::begin(paths), std::end(paths),
        file_exists_in_path)};
