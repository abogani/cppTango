...
{   
    ...
    client_addr* previous_client_addr = reinterpret_cast<client_addr*>(t.self()->get_value(Util::get_tssk_client_info()));
    ...
    try
    {
        ...
        if (d->objref() != nullptr)
        {
            ...
            // std::cout << "in BlackBox::client_call_interceptor: attaching client info @" << std::hex << a << std::dec << " to the thread" << std::endl;
    ...
    {
        // restore the previous client info (we might be handling a collocated call generated by a remote one)
        // std::cout << "in BlackBox::client_call_interceptor: restoring initial client info (exception context)" << std::endl;
        t.self()->set_value(Util::get_tssk_client_info(), previous_client_addr);
