...
class ServerConfigureEventTestSuite : public CxxTest::TestSuite
{
  ...
  public:
    ...
    DeviceProxy *create_and_check_device1_proxy()
    {
        ...
        TS_ASSERT_THROWS_NOTHING(res_ping_device1 = device1->ping(););
    ...
    DeviceProxy *create_and_check_admin_device_proxy()
    {
        ...
        TS_ASSERT_THROWS_NOTHING(res_ping_dserver = dserver->ping(););
    ...
    void exercise_subscription_event(DeviceProxy *device1)
    {
        ...
        TS_ASSERT_THROWS_NOTHING(device1->subscribe_event(attribute_name, Tango::USER_EVENT, &callback));
    ...
    std::vector<std::string> query_admin_device(DeviceProxy *dserver)
    {
        ...
        {
            TEST_LOG << endl
                     << "cxx_server_configure_zmq_ports: info[" + std::to_string(i) + "] => " +
                            string(result->svalue[i])
                     << endl;
    ...
    void test_03_device_server_zmq_event_invalid_event_port_configured(void)
    {
        ...
        TS_ASSERT_THROWS_ASSERT(
            device1->subscribe_event(attribute_name, Tango::USER_EVENT, &callback), Tango::DevFailed & e, erescue = e;);
    ...
    void test_04_device_server_zmq_event_invalid_heartbeat_port_configured(void)
    {
        ...
        device1 = new DeviceProxy(device1_name);
        Tango::DevFailed erescue;
        TS_ASSERT_THROWS_ASSERT(device1->ping(), Tango::DevFailed & e, erescue = e;);
        TS_ASSERT(erescue.errors.length() >= 1);
        ...
        DeviceProxy *dserver;
        dserver = new DeviceProxy(dserver_name);
        TS_ASSERT_THROWS_ASSERT(dserver->ping(), Tango::DevFailed & e, erescue = e;);
        TS_ASSERT(erescue.errors.length() >= 1);
        TS_ASSERT_EQUALS(std::string(erescue.errors[0].reason.in()), "API_DeviceNotExported");
        ...
        din << din_info;
        TS_ASSERT_THROWS_ASSERT(
            dout = dserver->command_inout("ZmqEventSubscriptionChange", din), Tango::DevFailed & e, erescue = e;);
        TS_ASSERT(erescue.errors.length() >= 1);
